WITH hes_anomalies AS (
    ...
    WHERE
        ebr."type" = 'EHes'
        AND NOT (
            da.diagnosticmethod IS NULL
            AND da.detecteddate1 IS NULL
        )
),
audits AS (
    -- audits for those anomalies, pulling out detecteddate1 and diagnosticmethod
    SELECT
        ra.auditable_id diagnosedanomalyid,
        to_timestamp(
            substring(
                ra.xmldata,
                '<DETECTEDDATE1>(.*)</DETECTEDDATE1>'
            ),
        ...
        ) :: int diagnosticmethod,
        ra.userid,
        ra.created_at,
        row_number() OVER (
            PARTITION BY ra.auditable_id
            ORDER BY
                ra.created_at DESC
        ) sortorder
    FROM
        rovaudit ra
        INNER JOIN hes_anomalies ha ON ha.diagnosedanomalyid = ra.auditable_id
        AND ra.auditable_type = 'Diagnosedanomaly'
),
audit_history AS (
    -- union current state to audits to create the full ordered timeline
    SELECT
        ha.diagnosedanomalyid,
        ha.detecteddate1,
        ha.diagnosticmethod,
...
change_history AS (
    ...
    FROM
        audit_history ah_from
        INNER JOIN audit_history ah_to ON ah_to.diagnosedanomalyid = ah_from.diagnosedanomalyid
        AND ah_to.sortorder = ah_from.sortorder - 1
),
relevant_changes AS (
    -- isolate changes to diagnostic variables and group userids by type
    SELECT
        ch.diagnosedanomalyid,
        CASE
            WHEN ch.userid IN ()--dev names
            OR ch.userid LIKE 'batch%'
            OR ch.userid LIKE 'dba%' THEN 'machine'
            ELSE 'user'
        END AS user_type,
        ch.change_timestamp,
    ...
    FROM
        change_history ch
    WHERE
        ch.detecteddate_from != ch.detecteddate_to
        OR ch.diagnosticmethod_from != ch.diagnosticmethod_to
        OR (
            ch.detecteddate_from IS NULL
            AND ch.detecteddate_to IS NOT NULL
        )
        OR (
            ch.diagnosticmethod_from IS NULL
            AND ch.diagnosticmethod_to IS NOT NULL
        )
)
SELECT
    rc.diagnosedanomalyid,
    zicd.exportid,
    da.status,
    rc.change_timestamp,
...
FROM
    ...
    INNER JOIN zicd ON zicd.zicdid = da.icd -- the change was made automatically
WHERE
    rc.user_type = 'machine' -- the diagnostic variable has not been changed since
    AND (
        da.detecteddate1 = rc.detecteddate_to
        OR da.diagnosticmethod = rc.diagnosticmethod_to
    )
